                    ...
                    pageInit: function() {
                        ...
                        $('.productItem').hide();
                         if (elevate.validateSession(self.currentStep)) {
							...
							if(elevate.lsData.meta.guid){
								self.guid = elevate.lsData.meta.guid;
							}
							if(elevate.lsData.deliveryInfo && elevate.lsData.ekycPassed){
								self.ekycPassed = elevate.lsData.ekycPassed;
								self.deliveryInfo = elevate.lsData.deliveryInfo;
							}else{
								elevate.redirectToPage('pre-qualified?id='+self.guid);
							}
                    ...
                    removePrequalifiedCustomer: function () {
                        ...
                        axios.post(apiEndpointURL_elevate + '/del-prequalified-customer', params)
                            .then((response) => {
                                var data = response.data;
                                if(data.status == 1){
                                    elevate.redirectToPage('thanks-pre-qualified?status='+self.payment_status+'&orderNumber='+$('#displayOrderNumber').val());
                                }else{
                                    toggleOverlay(false);
                                    $('#error').html("System error, please try again.");
                                    $('#status_mesage').html('');
