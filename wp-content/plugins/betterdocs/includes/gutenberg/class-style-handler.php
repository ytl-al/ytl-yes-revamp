        ...
        public function betterdocs_blocks_edit_post($hook)
        {
            ...
            if ($hook == 'post-new.php' || $hook == 'post.php') {
                wp_enqueue_script(
                    ...
                    filemtime(BETTERDOCS_DIR_PATH . 'public/js/style-handler.js'),
                    true
                );
                wp_localize_script('betterdocs-blocks-edit-post', 'betterdocs_style_handler', [
        ...
        public function enqueue_frontend_css()
        {
            ...
            if (!empty($post) && !empty($post->ID)) {
                ...
                if (file_exists($upload_dir['basedir'] . '/betterdocs-style/betterdocs-style-' . $post->ID . '.min.css')) {
                    wp_enqueue_style('betterdocs-block-style-' . $post->ID, $upload_dir['baseurl'] . '/betterdocs-style/betterdocs-style-' . $post->ID . '.min.css', [], substr(md5(microtime(true)), 0, 10));
                }
