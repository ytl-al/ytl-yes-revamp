...
class WPImport extends WP_Importer {
    ...
    private function import_sample_data( $posts, $action ) {
        ...
        foreach ( $posts as $post ) {
            ...
            $category_slug = isset($post['Category Slug']) ? $post['Category Slug'] : '';
            $knowledge_base_name = isset($post['Knowledge Base']) ? $post['Knowledge Base'] : '';
            $knowledge_base_slug = isset($post['Knowledge Base Slug']) ? $post['Knowledge Base Slug'] : '';
            ...
            if ( $default_multiple_kb == 1 ) {
                ...
                if (!$knowledge_base_id && $knowledge_base_name) {
                    $knowledge_base_id = wp_insert_term($knowledge_base_name, 'knowledge_base', array('slug' => $knowledge_base_slug));
                    $knowledge_base_id = $knowledge_base_id['term_id'];
                } elseif ( ! $knowledge_base_name ) {
                    // If no knowledge base term provided, set it to 0
                    $knowledge_base_id = 0;
                } else {
                    $knowledge_base_id = $knowledge_base_id['term_id'];
                }
            ...
            if ( ! $category_id ) {
                $category_id = wp_insert_term($category_name, 'doc_category', array('slug' => $category_slug));
            ...
            $post_data = [
                ...
                'post_type' => 'docs',
                'tax_input' => array(
                    'doc_category' => array( $category_id )
                ),
            ];

            if ( $default_multiple_kb == 1 ) {
                $post_data['tax_input']['knowledge_base'] = array( $knowledge_base_id );
            }
	...
	private function parse( $file ): array {
        ...
        } else if ($this->file_type == 'text/csv') {
