...
public function retrieve_supported_brands()
{
	...
	{
		if (!isset($this->api_domain)) {
			return new WP_Error('error_fetching_supported_devices', 'Parameters not complete to fetch supported devices', ['status' => 400]);
		}
		...
		$request = wp_remote_get($api_url, $args);
		if (is_wp_error($request)) {
			return new WP_Error('error_fetching_supported_devices', 'Error fetching supported devices', ['status' => 400]);
		}
	
		$response_code = wp_remote_retrieve_response_code($request);
		if ($response_code !== 200) {
			return new WP_Error('error_fetching_supported_devices', 'Failed to fetch supported devices', ['status' => $response_code]);
		}
